 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few numbers") {
    ...
    assert(sum(List(10,5,-10)) === 5)
    assert(sum(List(-1,-1,-1)) === -3)
  }

  test("max of a few numbers") {
    assert(max(List(3, 7, 2)) === 7)
    assert(max(List(4, 10, 11)) === 11)
    assert(max(List(-4, -40, 11)) === 11)
    assert(max(List(7, 5, 7)) === 7)
  }

  test("max throws an exception when its argument is empty") {
   intercept[NoSuchElementException] {
     listNotEmpty(List())
   }
  }

  def listNotEmpty(xs: List[Int]): List[Int] = {
    if (xs.isEmpty) throw new NoSuchElementException("input list cannot be empty")
    else xs
  }
